...
def main():
    ...
    parser = argparse.ArgumentParser() #! This function allows users to provide values for variables at runtime
...
def train(args):
    ...
    if not os.path.isdir("log/"):
      print("Directory creation script is running...")  #! Creating directories with the bash file
      subprocess.call([f_prefix+'/make_directories.sh'])
    #! numpy.clip(a, a_min, a_max) converts all values of a that are below a_min to be a_min and all values of a that are above a_max to be a_max
    #! Frequency number(epoch) of validation using validation data
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(dataloader.num_batches):
            ...
            for sequence in range(dataloader.batch_size):
                ...
                # substract first frame value to all frames for a pedestrian
