...
class DataLoader():
    ...
    def __init__(self,f_prefix, batch_size=5, seq_length=20, num_of_validation = 0, forcePreProcess=False, infer=False, generate = False):
        ...
        '''
        # base test files
        file_test_no = [0,14,18,2,25,29,3,30,33,39,4,42,45,57,61,62,63,65,69,7,71,9]
        base_test_dataset= ['/data/test/final/test_'+str(i)+'.txt' for i in file_test_no]
        #base train files
        file_train_no = list(set(range(72))-set(file_test_no))
        base_train_dataset= ['/data/train/final/train_'+str(i)+'.txt' for i in file_train_no]
        # dimensions of each file set
        self.dataset_dimensions = {'final':[720, 576]}
        ...
        for directory in data_dirs:
            ...
            if self.infer is False:
                ...
                # if test mode, read test file to pandas dataframe and process
                    ...
                    #print("dfx",df['x'])
                    #print("dfy",df['x'])
                    #print("target_ids",np.array(df[df['y'].isnull()].drop_duplicates(subset={'ped_id'}, keep='first', inplace=False)['ped_id']))
